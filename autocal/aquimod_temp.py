...
class AquiModAWS:
    ...
    def read_calibration_parameters(self) -> dict[str, pd.DataFrame]:
        """
        ...
        for component, path in self.calibration_paths.items():
            ...
            for parameter in self.parameters[component]:
                # Get min and max value for parameter as string
                minmax = self._read_line(path, self._parameter_line_numbers[parameter])
                # Convert string into list and cast elements into floats
    ...
    def read_performance_output(self) -> dict[str, pd.DataFrame]:
        """Read output files including parameter values and model performance"""
